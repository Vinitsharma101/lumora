...
# Allowlisted domains for stock media downloads (SSRF protection)
...
async def download_stock(body: StockDownloadRequest, request: Request):
    ...
    # SSRF protection: only allow downloads from known stock media hosts
    parsed = urlparse(body.url)
    if parsed.hostname not in _ALLOWED_DOWNLOAD_HOSTS:
        raise HTTPException(
            status_code=400,
            detail=f"Downloads only allowed from supported stock media providers",
        )
